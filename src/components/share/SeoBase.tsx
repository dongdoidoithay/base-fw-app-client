...
export function SeoBase({ current }: any) {
    ...
    if (current.Menu) {
         current.Menu?.map((item: any) => {
            let _link = `https://${current.setting?.domain_name}${UrlByKeyType(item.key_type, current)}`;
            if (item?.child.length > 0) {
                 item.child.map((item_child: any) => {
                    _index++;
                    _link = `https://${current.setting?.domain_name}${UrlByKeyType(item_child.key_type, current)}`;
                       

                        ItemBreadcrumb.push({
                            position: _index,
                            name: LableMenuName(item_child.key_type, current),
                            item: _link
                        });
                })
            }
    ...
    return <>
        <Head>
            ...
            <meta name="sitemap-api" content={`https://${current?.setting?.domain_name}/${current?.setting?.domain_name}/sitemap-index.xml`}></meta>
            <meta name="robots" content={`https://${current?.setting?.domain_name}/robots.txt`}></meta>
            <meta name="sitemap" content={`https://${current?.setting?.domain_name}/sitemap.xml`}></meta>
        </Head>
        ...
        <NextSeo
            ...
            additionalLinkTags={[
                {
                    rel: "alternate",
                    href: `https://${current?.setting?.domain_name}/api/rss.xml`,
                    type: "application/rss+xml",
                },
                {
                    rel: "alternate",
                    href: `https://${current?.setting?.domain_name}/api/ror.xml`,
                    type: "application/rss+xml",
                },
                {
                    rel: "alternate",
                    href: `https://${current?.setting?.domain_name}/api/sitemap.html`,
                    type: "text/html",
                },
                {
                    rel: "alternate",
                    href: `https://${current?.setting?.domain_name}/api/urllist.txt`,
                    type: "text/plain",
                },
                {
                    rel: "alternate",
                    href: `https://${current?.setting?.domain_name}/api/server-sitemap.xml`,
                    type: "application/rss+xml",
                },
            ]}
        />
