...
const Ads_Block_PopUp = dynamic(() => import('../ads/Ads_Block_PopUp'), { ssr: false });
const Ads_Block_Banner = dynamic(() => import('../ads/Ads_Block_Banner'), { ssr: false });
const Ads_AdsPubfuture_Sticky = dynamic(() => import('../ads/Ads_AdsPubfuture_Sticky'), { ssr: false });
...
export function GlobalNav({ current }: any) 
{
    ...
    function _renderChild(item:any){
        ...
        return (
            <li className={`${ClassChild(item.Path, item?.child?.length)}`} key={item.key_type}>
                <Link href={`${UrlByKeyType(item.key_type, current)}`} > {LableMenuName(item.key_type, current)}</Link>
            </li>
        )
    }
                            ...
                            <div className="col-md-12">
                                <div className="menu">
                                    ...
                                    <ul className={`nav-bar-menu ${navMenuCssClass}`} ref={ref}>
                                        ...
                                        {current?.Menu && current?.Menu.sort().map((item: any) => _renderChild(item))}
