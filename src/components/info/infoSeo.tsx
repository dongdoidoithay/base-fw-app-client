...
export function InfoSeo({ current, data, dataChapter }: any) {
    //console.log(dataChapter);
    const _title = TitleSeo({ text: data?.infoDoc.name, option: TypeDoc.Info, current: current, genres: data?.infoDoc.genresName, chapter: "1", auth: data?.infoDoc.authName, desc: data?.infoDoc.desc, nameother: data?.infoDoc.nameOther });
    const _desc = DescSeo({ text: data?.infoDoc.name, option: TypeDoc.Info, current: current, genres: data?.infoDoc.genresName, chapter: "1", auth: data?.infoDoc.authName, desc: data?.infoDoc.desc, nameother: data?.infoDoc.nameOther });
    const _keyword = KeyWordSeo({ text: data?.infoDoc.name, option: TypeDoc.Info, current: current, genres: data?.infoDoc.genresName, chapter: "1", auth: data?.infoDoc.authName, desc: data?.infoDoc.desc, nameother: data?.infoDoc.nameOther });
    const _urlDoc = `https://${current?.setting?.domain_name}${UrlInfo(data?.infoDoc.idDoc, current)}`;

    let ItemBreadcrumb: any = [];
    if (dataChapter?.data) {
        ItemBreadcrumb = dataChapter?.data?.map((item: any, indx: number) => {
            let _link = `https://${current?.setting?.domain_name}${UrlDetail(item.idDoc, item.idDetail, current)}`;
                return (
                    {
                        position: indx + 2,
                        name: item.nameDoc + ' - ' + item.nameChapter,
                        item: _link
                    }
                )
            }
    ...
    return <>
        ...
        <NextSeo
            ...
            additionalLinkTags={[{
                rel: "alternate",
                href: `https://${current?.setting?.domain_name}/api${UrlInfo(data?.infoDoc.idDoc, current)}/feed`,
                type: "application/rss+xml"
              },
              {
                rel: "alternate",
                href: `https://${current?.setting?.domain_name}/api${UrlInfo(data?.infoDoc.idDoc, current)}/rss.xml`,
                type: "application/rss+xml"
              }
              ]}
        />
